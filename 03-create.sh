...
if [ -f "$WORKDIR/lang.de" ]; then
	...
	cp $WORKDIR/isolinux/isolinux-de.cfg -O $IMAGEDIR/isolinux/isolinux.cfg
else
	cp $WORKDIR/isolinux/isolinux-en.cfg -O $IMAGEDIR/isolinux/isolinux.cfg
fi
cp $WORKDIR/isolinux/splash.png -O $IMAGEDIR/isolinux/splash.png

#copy preseed
if [ -f "$WORKDIR/lang.de" ]; then
	cp $WORKDIR/preseed/chooseyourown-de.seed -O $IMAGEDIR/preseed/chooseyourown.seed
	cp $WORKDIR/preseed/allinone-de.seed -O $IMAGEDIR/preseed/allinone.seed
	cp $WORKDIR/preseed/seperatehome-de.seed -O $IMAGEDIR/preseed/seperatehome.seed
else
	cp $WORKDIR/preseed/chooseyourown-en.seed -O $IMAGEDIR/preseed/chooseyourown.seed
	cp $WORKDIR/preseed/allinone-en.seed -O $IMAGEDIR/preseed/allinone.seed
	cp $WORKDIR/preseed/seperatehome-en.seed -O $IMAGEDIR/preseed/seperatehome.seed
fi
cp $WORKDIR/preseed/ubuntu.seed -O $IMAGEDIR/preseed/ubuntu.seed
