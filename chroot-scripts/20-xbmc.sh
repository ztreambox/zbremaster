...
if [ ! -f "/etc/apt/sources.list.d/pulse-eight-pulse-precise.list" ]; then
	apt-add-repository -y ppa:pulse-eight/pulse
fi
ISCONFED=`dpkg -s xbmc | grep 'install ok installed'`
if [ -z "$ISCONFED" ]; then
	apt-get update
	apt-get install -y --force-yes xbmc afpfs-ng-utils lcdproc libaacs0 libbluray1 libcec libcrystalhd3 libnfs1 librtmp0 libshairport1
